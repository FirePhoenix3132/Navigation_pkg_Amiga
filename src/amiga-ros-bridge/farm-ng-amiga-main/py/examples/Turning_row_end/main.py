...
async def build_track(save_track_flag: bool, clients: dict[str, EventClient] | None = None, save_track: Path | None = None) -> Track:
    ...
    row_spacing: float = 3*12 * 0.0254  # 36 inches in meters
    extended_length: float = 24 * 12 * 0.0254  # 3 feet in meters
    ...
    # Maneuver at the end of row: skip one row (72 inches) - (go from 2 to 4)
    track_builder.create_arc_segment(next_frame_b="goal4", radius=row_spacing, angle=radians(-180), spacing=0.1)
...
async def run(args) -> None:
    ...
    save_track_flag: bool = False #If path is given, then its true. For simplicity you can change this to True or False.
    # save_track =args.save_track
    save_track = Path('~/catkin_ws_amiga/src/amiga-ros-bridge/farm-ng-amiga-main/py/examples/Turning_row_end/')
    ...
    tasks: list[asyncio.Task] = [
        asyncio.create_task(start_track(clients,save_track_flag, save_track)),
